...
`jd_keywords`、`matched_keywords` 和 `missing_keywords` 表示技能关键词；`responsibility_keywords`、`matched_responsibilities` 和 `missing_responsibilities` 表示职责关键词。两组匹配与未匹配字段各自完整划分对应岗位关键词。技能和职责标签来自简历原文的确定性规则识别；Hybrid 模式的经历相关性分数由 AI 结合可验证原文证据独立评估，规则职责标签不等同于模型分数明细。AI 不可用时，职责覆盖率才直接用于经历分降级计算。
