...
| T-PDF-001A | PDF 包含跨页或定位式项目区块 | 布局感知提取保持项目标题、描述和亮点连续，不把下一项目标题串入上一项目 | FR-PDF-002 |
...
| T-PROFILE-103 | 简历包含教育和项目经历 | 返回结构化数组；项目保留日期、描述、职责亮点和技术栈，缺失时为空数组 | FR-AI-103 |
...
| T-MATCH-101 | AI 返回经历相关性和证据 | 分数在 0 至 100；允许模型归一换行空白，但最终返回的证据必须回定位为简历原文片段 | FR-MATCH-101 |
...
| T-UI-004 | 核心字段缺失 | 对应位置显示“未提取到”等中性文案，页面不报错 | FR-UI-001 |
...
阶段 3 已正式挂载 `POST /api/v1/resumes`。T-PDF-001 的公共端点 `201` 验收必须同时证明：文件元数据来自本次解析结果、档案字段完整、清洗文本未截断、降级警告语义正确且 `cached=false`。集成测试使用可控提取器或缺配置规则降级，不访问真实模型。
