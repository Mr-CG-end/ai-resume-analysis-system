...
本阶段完成可信的 multipart 单文件适配、PDF 三重类型校验、大小与页数限制、pypdf 多页文本提取、保守文本清洗、内容哈希和统一错误映射。所有功能必须能在无 AI、无 Redis、无网络环境下确定性测试。
...
| 文件大小 | 10 MB 接受，10 MB + 1 字节返回 `413 PDF_TOO_LARGE`，不进入 pypdf |
...
| 处理预算 | 单流或累计内容流超过 50 MiB、原始提取文本超过 1,000,000 字符返回 `422 PDF_PROCESSING_LIMIT_EXCEEDED` |
| 多页提取 | 三页按页序合并；`page_count`、`character_count`、SHA-256 正确 |
| 文本清洗 | 归一 CRLF/NBSP/水平空白；删除跨页高频边界页眉页脚；不重排正文 |
| 资源与隐私 | UploadFile、pypdf reader 和内存流在所有路径关闭；日志/响应不含正文、文件名、PII 或内部异常 |
...
- 在运行依赖中加入 `pypdf` 和 `python-multipart`，ReportLab 仅用于生成脱敏测试资料；同步哈希锁、基础设施校验与 Docker 安装验证。
...
- 使用 `PdfReader(BytesIO(...), strict=True)`；拒绝所有加密文档，仅映射已知 pypdf 读取错误，不吞掉未知编程错误。
- 进程级限制 pypdf 单流解压为 50 MiB，应用层累计跨页内容流 50 MiB、原始提取文本 1,000,000 字符，并与清洗后的 100,000 字符业务限制使用不同错误码。
- `clean_pages()` 为纯函数；只删除多页首尾区域的高频重复行和明确页码，不全局改写正文。

### 3. multipart 适配与统一错误

- 使用 `request.form()` 显式检查全部文件 part，不依赖单个 `UploadFile` 参数折叠重复字段。
- 缺少 boundary、字段数超限或 multipart 语法错误统一返回 `400 MALFORMED_MULTIPART`；只有实际请求体或文件超限返回 `413 PDF_TOO_LARGE`。
