...
系统需要在 24 小时内完成并公开部署，因此设计重点是清晰的模块边界、稳定的接口和可验证的降级行为，而不是堆叠基础设施。后端使用 Python、FastAPI 和 Pydantic，PDF 文本提取使用 PyMuPDF；前端使用 React、TypeScript 和 Vite。AI 服务通过兼容结构化 JSON 输出的 HTTP API 接入，具体供应商和模型由环境变量配置。
...
上传接口先进行大小检查，再检查扩展名、MIME 和 `%PDF-` 文件头。PyMuPDF 只在校验通过后打开文件，以页为单位提取文本并记录页数。加密、损坏、超过 30 页或没有有效文本的文档在这一阶段结束，不能继续消耗 AI 调用额度。
...
缓存键必须包含算法版本，避免 Prompt、字段或评分公式调整后复用旧数据：
