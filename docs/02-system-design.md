...
JD 服务输出技能关键词和职责关键词，执行大小写归一、首尾空白清理和去重。岗位侧使用严格职责词典，简历侧使用独立的证据词典识别“前端项目负责人”“优化构建速度”等明确近义表达；简历证据词典不收录单独的“开发”“优化”等宽泛词，避免误判。技能分由确定性公式计算：

```text
skill_match = matched_skill_count / jd_skill_count × 100
```

经历相关性由 AI 在 0 至 100 范围内评分，并必须返回来自简历的证据片段。评分器只接受确实存在于 `cleaned_text` 中的证据；无法验证的证据被丢弃。职责标签始终来自可复核的规则证据，不由模型改写。AI 不可用时，经历分退回职责关键词覆盖率，并设置 `method=rule_fallback`。
