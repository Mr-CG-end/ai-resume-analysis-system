...
阶段 2 的 HTTP 集成测试通过临时 FastAPI 应用挂载内部测试路由，只验证 multipart 枚举、大小边界、资源关闭、同步 PDF 服务线程池调用和统一错误映射。正式应用在本阶段不得挂载 `POST /api/v1/resumes` 的成功路由；阶段 3 生成真实 `CandidateProfile` 并组装完整 `ResumeSnapshot` 后，再执行 T-PDF-001 的公共端点 `201` 验收。

## API 契约测试

契约测试使用固定 JSON fixture 验证：所有 `CandidateProfile` 字段始终存在；数组不会变成 `null`；错误结构包含 `code`、`message` 和 `request_id`；分数是 0 至 100 的整数；`method` 只出现允许值；新增后端字段不会导致前端解析失败。
