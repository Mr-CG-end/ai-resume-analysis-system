...
- 环境待办：本机 Python 为 3.12.3，CI/镜像固定 3.12.13；Docker Desktop daemon 未运行，未实建 `linux/amd64` 镜像；托管 GitHub Actions 尚未触发。以上事项须在后续部署交付中补验。
...
- [x] 编写阶段 3 实施计划和验收矩阵。
- [ ] 完成公共响应与内部证据 Schema。
- [ ] 完成证据过滤、电话邮箱规则降级和工作年限计算。
- [ ] 完成 OpenAI 兼容客户端、两次调用预算和安全错误分类。
- [ ] 完成 AI 配置、健康检查与生产依赖锁。
- [ ] 正式挂载 `POST /api/v1/resumes` 并完成隐私日志。
- [ ] 完成后端、基础设施和前端回归。
- [ ] 完成实现、API/隐私和交付三路独立终审。
