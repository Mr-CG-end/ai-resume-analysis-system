...
当前仓库仍处于工程骨架阶段，只能执行“当前骨架核验”。当前前端仅展示流程说明和禁用的上传按钮，后端仅提供健康检查；PDF 上传与解析、AI 信息提取、JD 匹配评分及对应交互尚未实现。通过骨架核验不等于项目最终验收通过。
...
2. 打开终端中 Vite 输出的本地地址（通常为 `http://localhost:5173`）。
   - 页面显示“智能简历分析系统”和三步流程说明。
   - “上传 PDF 简历（下一步实现）”按钮处于禁用状态。
   - 页面明确说明 PDF 上传功能尚未实现；不要尝试把该页面当作完整业务流程验收。
3. 打开浏览器开发者工具。
   - 页面加载过程中没有未处理的 JavaScript 异常。
   - 页面可使用键盘滚动，文字和禁用状态清晰可辨。

完成以上步骤后，只能记录“当前骨架核验通过”，不得勾选项目最终人工验收。
